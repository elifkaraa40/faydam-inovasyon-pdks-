...
class _AttendanceCorrectionScreenState extends State<AttendanceCorrectionScreen> {
  ...
  @override Widget build(BuildContext context) {
    final dark = context.watch<AppSettings>().isDarkMode; final text = dark ? Colors.white : AppColors.darkNavy;
    return Scaffold(backgroundColor: dark ? AppColors.darkNavy : AppColors.lightBackground, appBar: AppBar(title: const Text('Puantaj düzeltme'), backgroundColor: Colors.transparent, elevation: 0), body: RefreshIndicator(onRefresh: _load, child: ListView(padding: const EdgeInsets.all(16), children: [
      Card(color: dark ? AppColors.cardNavy : Colors.white, child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        const Text('Eksik veya hatalı kayıt'), const SizedBox(height: 12),
        ...
        Row(children: [Expanded(child: TextField(controller: _entry, decoration: const InputDecoration(labelText: 'Giriş', prefixIcon: Icon(Icons.login)), keyboardType: TextInputType.datetime)), const SizedBox(width: 12), Expanded(child: TextField(controller: _exit, decoration: const InputDecoration(labelText: 'Çıkış', prefixIcon: Icon(Icons.logout)), keyboardType: TextInputType.datetime))]),
